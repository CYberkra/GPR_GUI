...
def updatabwd(Ey, Hx, Hz, Dbz, Dbx, Bz, Bx, Az, Ax, Dc, Ca, Cbx, Cbz, Cc, PEyx, PEyz, PHx, PHz, Eydiffz, Eydiffx,
              ...
              l3, kp3, lp3, ki3, ip3, jp3, ii3, numit, srci, data1, npml, g, endpic, image3, image4, maxsource, nq, m1, s1):
    ...
    for it in range(numit):
        ...
        for v in range(maxsource):
            ...
            Ey[i, j, v] += data1[numit - it - 1, (s1 - 1) * maxsource + v]

        for v in range(Ey.shape[2]):
            image3[:, :, v] = endpic[:, :, numit - it - 1] * Ey[:, :, v] + image3[:, :, v]
            image4[:, :, v] = endpic[:, :, it] * endpic[:, :, it] + image4[:, :, v]

    return image3, image4

def updatabwd_ini(Ey, Hx, Hz, Dbz, Dbx, Bz, Bx, Az, Ax, Dc, Ca, Cbx, Cbz, Cc, PEyx, PEyz, PHx, PHz, Eydiffz, Eydiffx,
              Hxdiffz, Hzdiffx,i1, j1, k1, l1, kp1, lp1, ki1, ip1, jp1, ii1, i2, j2, k2, l2, kp2, lp2, ki2, ip2, jp2, ii2, i3, j3, k3,
              l3, kp3, lp3, ki3, ip3, jp3, ii3, numit, srci, data1, npml, g, endpic, image3, image4, maxsource, nq, m1, s1):
    # 添加额外的维度以匹配 Ey, Hx, Hz等数组
    i1 = i1[:, cp.newaxis, cp.newaxis]  # i1 shape: (len, 1, 1)
    ...
    for it in range(numit):
        ...
        for v in range(Ey.shape[2]):
            image3[:, :, v] = endpic[:, :, numit - it - 1,v] * Ey[:, :, v] + image3[:, :, v]
            image4[:, :, v] = endpic[:, :, it,v] * endpic[:, :, it,v] + image4[:, :, v]
